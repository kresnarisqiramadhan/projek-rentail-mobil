<nav class="bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl docked full-width top-0 sticky z-50 border-b border-zinc-100/50 dark:border-zinc-800/50 shadow-[0_4px_30px_rgba(0,0,0,0.03)]">
    <div class="flex justify-between items-center h-16 px-6 md:px-12 max-w-[1440px] mx-auto">
        ...
        <div class="hidden md:flex space-x-8 items-center font-manrope font-light tracking-tight text-sm absolute left-1/2 -translate-x-1/2">
            <a class="text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-50 transition-colors" href="{{ route('vehicles') }}">{{ __('Vehicles') }}</a>
            <a class="text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-50 transition-colors" href="{{ route('experience') }}">{{ __('Experience') }}</a>
            <a class="text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-50 transition-colors" href="{{ route('locations') }}">{{ __('Locations') }}</a>
            ...
            <div class="flex items-center space-x-2">
                @auth
                    ...
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="px-4 py-2 text-xs font-medium bg-zinc-900 dark:bg-zinc-50 text-white dark:text-zinc-900 rounded-full hover:opacity-90 transition-all shadow-sm">
                        {{ __('Logout') }}
                        </button>
                    </form>
                @else
